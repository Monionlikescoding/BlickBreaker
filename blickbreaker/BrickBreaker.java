...
public class BrickBreaker implements ActionListener, KeyListener {
    ...
    public void initializeSprites() {
        ...
        Brick b = new AcceleratorBrick(x+200, y+60, 50, 20, 3, 200);
        bricks.add(b);
        Brick c = new DeceleratorBrick(x+200, y+80, 50, 20, 3, 200);
        bricks.add(c);
        
        for (int i = 0; i<4;i++) {
            Brick a = new ToughBrick(x+50+(100*i), y+20, 50, 20, 2, 200);
            bricks.add(a);
        }
        
        Brick d = new TeleportingBrick(x+200, y+120, 50, 20, 3, 200);
        bricks.add(d);
        for (int i = 0; i<3;i++) {    
            Brick e = new MovingBrick(x+200, y+80 + (i*20), 50, 20, 3, 200);
            bricks.add(e);
        }
    }
    ...
    public void renderOffScreen(Graphics2D g) {
        ...
        if (bricksLeftToWin == 0) {
            if (levels == 2) {
                ...
                Brick b = new AcceleratorBrick(x+200, y+60, 50, 20, 10, 200);
                bricks.add(b);
                Brick c = new DeceleratorBrick(x+350, y+60, 50, 20, 3, 200);
                bricks.add(c);
                Brick d = new DeceleratorBrick(x+150, y+60, 50, 20, 3, 200);
                bricks.add(d);
                for (int i = 0; i<4;i++) {
                    Brick a = new ToughBrick(x+50+(100*i), y+20, 50, 20, 6, 200);
                    bricks.add(a);
                }
                for (int i = 0; i<4;i++) {
                    Brick a = new MovingBrick(x+50+(100*i), y+160, 50, 20, 6000, 200);
                    bricks.add(a);
                }
                for (int i = 0; i<8;i++) {
                    Brick a = new TeleportingBrick(x+(50*i), y, 50, 20, 6, 200);
                    bricks.add(a);
                }
            }
        }
        ...
        if (ball.getY() + ball.getHeight() + ball.getDy() > HEIGHT) {
            ...
            if (lives > 0) {
                ...
                g.setFont(theFont);
                 int start = (WIDTH - g.getFontMetrics().stringWidth("⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡿⠟⠛⠛⠛⠋⠉⠉⠉⠉⠉⠉⠉⠉⠉⠉⠉⠉⠉⠙⠛⠛⠛⠿⠻⠿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿")) / 2;
                g.drawString("⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡿⠟⠛⠛⠛⠋⠉⠉⠉⠉⠉⠉⠉⠉⠉⠉⠉⠉⠉⠙⠛⠛⠛⠿⠻⠿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿", start, HEIGHT / 3);
                g.drawString("⣿⣿⣿⣿⣿⣿⣿⣿⣿⡿⠋⠀⠀⠀⠀⠀⡀⠠⠤⠒⢂⣉⣉⣉⣑⣒⣒⠒⠒⠒⠒⠒⠒⠒⠀⠀⠐⠒⠚⠻⠿⠿⣿⣿⣿⣿⣿⣿⣿⣿", start, (HEIGHT / 3)+10);
                g.drawString("⣿⣿⣿⣿⣿⣿⣿⣿⠏⠀⠀⠀⠀⡠⠔⠉⣀⠔⠒⠉⣀⣀⠀⠀⠀⣀⡀⠈⠉⠑⠒⠒⠒⠒⠒⠈⠉⠉⠉⠁⠂⠀⠈⠙⢿⣿⣿⣿⣿⣿", start, (HEIGHT / 3)+20);
                g.drawString("⣿⣿⣿⣿⣿⣿⣿⠇⠀⠀⠀⠔⠁⠠⠖⠡⠔⠊⠀⠀⠀⠀⠀⠀⠀⠐⡄⠀⠀⠀⠀⠀⠀⡄⠀⠀⠀⠀⠉⠲⢄⠀⠀⠀⠈⣿⣿⣿⣿⣿", start, (HEIGHT / 3)+30);
                g.drawString("⣿⣿⣿⣿⣿⣿⠋⠀⠀⠀⠀⠀⠀⠀⠊⠀⢀⣀⣤⣤⣤⣤⣀⠀⠀⠀⢸⠀⠀⠀⠀⠀⠜⠀⠀⠀⠀⣀⡀⠀⠈⠃⠀⠀⠀⠸⣿⣿⣿⣿", start, (HEIGHT / 3)+40);
                g.drawString("⣿⣿⣿⣿⡿⠥⠐⠂⠀⠀⠀⠀⡄⠀⠰⢺⣿⣿⣿⣿⣿⣟⠀⠈⠐⢤⠀⠀⠀⠀⠀⠀⢀⣠⣶⣾⣯⠀⠀⠉⠂⠀⠠⠤⢄⣀⠙⢿⣿⣿", start, (HEIGHT / 3)+50);
                g.drawString("⣿⡿⠋⠡⠐⠈⣉⠭⠤⠤⢄⡀⠈⠀⠈⠁⠉⠁⡠⠀⠀⠀⠉⠐⠠⠔⠀⠀⠀⠀⠀⠲⣿⠿⠛⠛⠓⠒⠂⠀⠀⠀⠀⠀⠀⠠⡉⢢⠙⣿", start, (HEIGHT / 3)+60);
                g.drawString("⣿⠀⢀⠁⠀⠊⠀⠀⠀⠀⠀⠈⠁⠒⠂⠀⠒⠊⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⡇⠀⠀⠀⠀⠀⢀⣀⡠⠔⠒⠒⠂⠀⠈⠀⡇⣿", start, (HEIGHT / 3)+70);
                g.drawString("⣿⠀⢸⠀⠀⠀⢀⣀⡠⠋⠓⠤⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠄⠀⠀⠀⠀⠀⠀⠈⠢⠤⡀⠀⠀⠀⠀⠀⠀⢠⠀⠀⠀⡠⠀⡇⣿", start, (HEIGHT / 3)+80);
                g.drawString("⣿⡀⠘⠀⠀⠀⠀⠀⠘⡄⠀⠀⠀⠈⠑⡦⢄⣀⠀⠀⠐⠒⠁⢸⠀⠀⠠⠒⠄⠀⠀⠀⠀⠀⢀⠇⠀⣀⡀⠀⠀⢀⢾⡆⠀⠈⡀⠎⣸⣿", start, (HEIGHT / 3)+90);
                g.drawString("⣿⣿⣄⡈⠢⠀⠀⠀⠀⠘⣶⣄⡀⠀⠀⡇⠀⠀⠈⠉⠒⠢⡤⣀⡀⠀⠀⠀⠀⠀⠐⠦⠤⠒⠁⠀⠀⠀⠀⣀⢴⠁⠀⢷⠀⠀⠀⢰⣿⣿", start, (HEIGHT / 3)+100);
                g.drawString("⣿⣿⣿⣿⣇⠂⠀⠀⠀⠀⠈⢂⠀⠈⠹⡧⣀⠀⠀⠀⠀⠀⡇⠀⠀⠉⠉⠉⢱⠒⠒⠒⠒⢖⠒⠒⠂⠙⠏⠀⠘⡀⠀⢸⠀⠀⠀⣿⣿⣿", start, (HEIGHT / 3)+110);
                g.drawString("⣿⣿⣿⣿⣿⣧⠀⠀⠀⠀⠀⠀⠑⠄⠰⠀⠀⠁⠐⠲⣤⣴⣄⡀⠀⠀⠀⠀⢸⠀⠀⠀⠀⢸⠀⠀⠀⠀⢠⠀⣠⣷⣶⣿⠀⠀⢰⣿⣿⣿", start, (HEIGHT / 3)+120);
                g.drawString("⣿⣿⣿⣿⣿⣿⣧⠀⠀⠀⠀⠀⠀⠀⠁⢀⠀⠀⠀⠀⠀⡙⠋⠙⠓⠲⢤⣤⣷⣤⣤⣤⣤⣾⣦⣤⣤⣶⣿⣿⣿⣿⡟⢹⠀⠀⢸⣿⣿⣿", start, (HEIGHT / 3)+130);
                g.drawString("⣿⣿⣿⣿⣿⣿⣿⣧⡀⠀⠀⠀⠀⠀⠀⠀⠑⠀⢄⠀⡰⠁⠀⠀⠀⠀⠀⠈⠉⠁⠈⠉⠻⠋⠉⠛⢛⠉⠉⢹⠁⢀⢇⠎⠀⠀⢸⣿⣿⣿", start, (HEIGHT / 3)+140);
                g.drawString("⣿⣿⣿⣿⣿⣿⣿⣿⣿⣦⣀⠈⠢⢄⡉⠂⠄⡀⠀⠈⠒⠢⠄⠀⢀⣀⣀⣰⠀⠀⠀⠀⠀⠀⠀⠀⡀⠀⢀⣎⠀⠼⠊⠀⠀⠀⠘⣿⣿⣿", start, (HEIGHT / 3)+150);
                g.drawString("⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣄⡀⠉⠢⢄⡈⠑⠢⢄⡀⠀⠀⠀⠀⠀⠀⠉⠉⠉⠉⠉⠉⠉⠉⠉⠉⠁⠀⠀⢀⠀⠀⠀⠀⠀⢻⣿⣿", start, (HEIGHT / 3)+160);
                g.drawString("⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣦⣀⡈⠑⠢⢄⡀⠈⠑⠒⠤⠄⣀⣀⠀⠉⠉⠉⠉⠀⠀⠀⣀⡀⠤⠂⠁⠀⢀⠆⠀⠀⢸⣿⣿", start, (HEIGHT / 3)+170);
                g.drawString("⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣦⣄⡀⠁⠉⠒⠂⠤⠤⣀⣀⣉⡉⠉⠉⠉⠉⢀⣀⣀⡠⠤⠒⠈⠀⠀⠀⠀⣸⣿⣿", start, (HEIGHT / 3)+180);
                g.drawString("⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣶⣤⣄⣀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣰⣿⣿⣿", start, (HEIGHT / 3)+190);
                g.drawString("⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣶⣶⣶⣶⣤⣤⣤⣤⣀⣀⣤⣤⣤⣶⣾⣿⣿⣿⣿⣿", start, (HEIGHT / 3)+200);
            }
            else {
                // Lose
                gameTimer.stop();
        ...
        g.setFont(theFont);
        int start = g.getFontMetrics().stringWidth("Lives : " + lives) - 10;
